...
# Seulement les employés qui ont besoin de données
targets = ["rahma", "rania", "Rihem  Ben Maouia", "Rihem bh Ben Maouia"]
# Super Admin est ignoré (pas besoin de pointage)

resp = requests.get(f"{SUPABASE_URL}/rest/v1/users?select=id,nom,prenom,company_id", headers=HEADERS)
users = [u for u in resp.json() if f"{u.get('prenom','')} {u.get('nom','')}".strip() in targets]
print(f"{len(users)} employés à traiter")
